 ...
 * 
 * This data provider allows you to define the list by JPQL count and query strings.
 * 
 * @param <T>
 *            The class of the objects this data provider provides
 */
public class JpqlQueryDataProvider<T extends HasId> implements IDataProvider<T> {
	 ...
	 *            The class of the objects this data provider provides
	 */
	public JpqlQueryDataProvider(final Class<T> clazz) {
	...
	public Iterator<? extends T> iterator(final int first, final int count) {
		return genericObjectLoader.getList(clazz, listQuery, first, count, queryParameters).iterator();
	}
	...
	public void setListQuery(final String listQuery) {
		this.listQuery = listQuery;
	}

	@Override
	public int size() {
		return (int) genericObjectLoader.count(countQuery, queryParameters);
	}
